...
function Releases({ releases }: { releases: PlayerRelease[] }) { const player = usePlayer(); return <section><SectionHeading label="Collections" title="Releases" /><div className="mt-6 grid gap-x-6 gap-y-12 sm:grid-cols-2">{releases.length ? releases.map((release, index) => <article key={release._id} className={`min-w-0 ${index % 2 ? 'sm:pt-12' : ''}`}><MediaArtwork src={release.coverArtUrl} size="feature" className="w-full max-w-none shadow-none" /><ProtocolLabel className="mt-4 text-[var(--text-muted)]">{release.releaseType || 'Release'}</ProtocolLabel><h3 className="mt-2 truncate text-xl font-semibold text-[var(--text-primary)]">{release.slug ? <Link href={`/releases/${release.slug}`} className="focusable-surface hover:text-[var(--accent)]">{release.title}</Link> : release.title}</h3><p className="type-metadata mt-2">{release.beats.length ? `${release.beats.length} playable tracks` : 'No playable tracks'}</p><div className="mt-4 flex flex-wrap items-center gap-x-5"><button type="button" disabled={!release.beats.length} onClick={() => void player.playQueue(release.beats, { type: 'release', title: release.title }, 0, false)} className="action-control focusable-surface"><Play className="h-4 w-4" fill="currentColor" /> {release.beats.length ? 'Play Release' : 'Release unavailable'}</button>{release.slug ? <Link href={`/releases/${release.slug}`} className="editorial-link focusable-surface">Open Release <ArrowUpRight className="h-4 w-4" /></Link> : null}</div></article>) : <p className="type-small border-y border-[var(--line-subtle)] py-6">No releases have been published.</p>}</div></section>; }
