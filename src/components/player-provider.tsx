...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const paused = () => { lastPositionUpdateRef.current = 0; dispatch({ type: 'paused' }); };
    const seeked = () => { lastPositionUpdateRef.current = 0; update(); };
    const error = () => dispatch({ type: 'error', message: 'The audio file could not be loaded. The signed URL may have expired.' });
    audio.addEventListener('loadedmetadata', ready); audio.addEventListener('durationchange', update); audio.addEventListener('timeupdate', update); audio.addEventListener('playing', playing); audio.addEventListener('pause', paused); audio.addEventListener('seeked', seeked); audio.addEventListener('ended', ended); audio.addEventListener('error', error);
