...
export function BeatLibrary({ beats, contextType, contextTitle }: { beats: PlayerBeat[]; contextType: 'main-library' | 'all-beats'; contextTitle: string }) {
  ...
  return <div className="border-t border-[var(--line-subtle)]">{beats.map((beat, index) => { const selected = player.beat?._id === beat._id; const cover = beat.coverArtUrl || beat.lane?.fallbackCoverArtUrl; return <article key={beat._id} className={`grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b border-[var(--line-subtle)] py-3 sm:grid-cols-[2rem_auto_minmax(0,1fr)_auto] sm:gap-4 ${selected ? 'bg-white/[0.025] shadow-[inset_2px_0_0_var(--accent)]' : ''}`}>
    <span className="type-numeric hidden w-7 text-center sm:block">{String(index + 1).padStart(2, '0')}</span>{beat.slug ? <Link href={`/player/beats/${beat.slug}`} aria-label={`Open Beat File for ${beat.title}`} className="artwork-link focusable-surface"><MediaArtwork src={cover} size="row" /></Link> : <MediaArtwork src={cover} size="row" />}
